...
| 端 | 入口 | 结构 |
|----|------|------|
| 客户首页 | `/home` | **我的活动权益** + **热门活动（广告同壳）** |
| 客户券包 | `/wallet` | 活动分组 → 展开权益 |
| 企业/店员 | `/business/offers` 底栏「活动券」 | 活动 → 权益模版/产品 + 操作（发券/核销/顾客页） |

共享：`src/lib/activity-entitlements.ts` · `ActivityEntitlementCard`
