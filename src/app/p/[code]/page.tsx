...
export default async function PromoLandingPage({ params }: { params: Promise<{ code: string }> }) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center px-6">
          ...
          <h1 className="text-lg font-semibold text-foreground">链接已失效</h1>
          <p className="text-sm text-muted-foreground mt-2">该推广链接不存在或已下线</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#FF6B35] via-orange-50 dark:via-background to-white">
      {/* 推广者标签 */}
      ...
      <div className="px-4 mt-4">
        <Card className="max-w-sm mx-auto overflow-hidden border-0 shadow-lg">
          ...
          <CardContent className="p-4 space-y-2">
            <div className="flex justify-between text-xs"><span className="text-muted-foreground">所需积分</span><span className="text-[#FF6B35] font-bold">{coupon.pointsRequired}⭐</span></div>
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">有效期</span>
              <span className={daysLeft <= 3 ? "text-red-500" : "text-foreground"}>{coupon.validUntil.toLocaleDateString("zh-CN")} ({daysLeft}天)</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">推广奖励</span>
      ...
      <div className="px-6 mt-6 max-w-sm mx-auto">
        ...
        <div className="mt-6 p-4 bg-card rounded-xl border border-border">
          <p className="text-xs text-muted-foreground text-center mb-2">💡 你也可以成为推广者</p>
