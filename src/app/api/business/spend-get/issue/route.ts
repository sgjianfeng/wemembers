// POST /api/business/spend-get/issue
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 列出可用满赠活动（holiday 或带 category:spend_get 标签）
    ...
    return NextResponse.json({
      data: {
        ...
        campaigns: campaigns.map((c) => {
          const rules = parseSpendGetRules(c.rulesSnapshot);
          return {
            id: c.id,
            name: c.name,
            type: c.type,
    ...
    console.error("spend-get issue GET", e);
  ...
  try {
    ...
    try {
      ...
      if (known.includes(code)) {
        return NextResponse.json(
          { error: spendGetIssueErrorMessage(code, "zh"), code },
          { status: 400 }
        );
      }
      console.error("spend-get issue POST tx", err);
      return NextResponse.json({ error: "发放失败" }, { status: 500 });
    }
  } catch (e) {
    console.error("spend-get issue POST", e);
    return NextResponse.json({ error: "发放失败" }, { status: 500 });
  }
}
