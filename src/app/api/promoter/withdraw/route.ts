...
export async function POST(request: NextRequest) {
  ...
  if (account.availableBalance < amount * 100) return NextResponse.json({ error: `可提现余额不足（可用 S$${(account.availableBalance / 100).toFixed(2)}）` }, { status: 400 });
  if (amount < 10) return NextResponse.json({ error: "最低提现金额为 S$10.00" }, { status: 400 });
  ...
  return NextResponse.json({
    data: {
      ...
      message: `提现 S$${amount.toFixed(2)} 申请已提交，预计1-3个工作日到账`,
