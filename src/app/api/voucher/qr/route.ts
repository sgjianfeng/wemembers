...
// Payload prefers short code: wmv:{shortCode} (fallback cuid)
import { NextRequest, NextResponse } from "next/server";
import { getSession } from "@/lib/auth";
import { prisma } from "@/lib/db";
import { generateQrCodeSvg } from "@/lib/qr";
import { ensureVoucherShortCode } from "@/lib/voucher-short-code";
...
export async function GET(request: NextRequest) {
  ...
  let code = voucher.shortCode;
  if (!code) {
    try {
      code = await ensureVoucherShortCode(voucher.id);
    } catch {
      code = voucher.id;
    }
  }

  // Short code preferred so staff can also type what they scan-read
  const payload = `wmv:${code}`;
