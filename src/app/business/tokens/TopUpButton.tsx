  ...
  if (!open) {
    return (
      <button
        ...
        className="flex-1 py-3 bg-primary text-primary-foreground rounded-full font-medium text-sm"
      >
        💳 充值
      </button>
    );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30 flex items-end" onClick={() => setOpen(false)}>
      <div className="w-full bg-card rounded-t-2xl p-5 max-w-lg mx-auto" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-sm font-semibold text-foreground mb-1">充值金额 (SGD)</h3>
        <p className="text-xs text-muted-foreground mb-3">支持信用卡和 PayNow</p>
          ...
          {QUICK_AMOUNTS.map((a) => (
            <button
              ...
              className={`py-2 rounded-xl text-sm font-medium transition-colors ${
                amount === a ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"
              }`}
            >
              S${a}
            </button>
