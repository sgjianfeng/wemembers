...
/**
 * 满赠配置页（企业主）：一键默认活动 + 可选全局发券
 * 现场发券请走「本店活动券 → 满赠操作台」/business/spend-get-desk?storeId=
 */
...
export default function SpendGetIssueSetupPage() {
  const { lang } = useLang();
  const zh = lang !== "en";
  ...
  useEffect(() => {
    if (qStore) {
      window.location.replace(
        `/business/spend-get-desk?storeId=${encodeURIComponent(qStore)}`
      );
    }
  }, [qStore]);
  ...
  const load = useCallback(async () => {
    const [setupRes, sRes] = await Promise.all([
      fetch("/api/business/default-activities"),
  ...
  async function runSetup() {
    ...
    setErr(null);
    try {
      const res = await fetch("/api/business/default-activities", {
        method: "POST",
      ...
      setMsg(
        zh
          ? "默认活动已就绪。请到「门店 → 本店活动券 → 满赠操作台」做现场发券。"
          : "Defaults ready. Use Store → Offers → Spend & get desk for counter work."
      );
      await load();
    } catch {
      ...
      <div className="px-4 py-4 border-b border-border flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold">
            {zh ? "满赠配置" : "Spend & get setup"}
          </h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            {zh
              ? "企业主：开启默认活动。现场发券请进本店操作台。"
              : "Owner: enable defaults. Counter work uses store Spend & get desk."}
          </p>
        </div>
          ...
          <Card>
            <CardContent className="p-4 space-y-2">
              ...
              {stores.map((s) => (
                <Link
                  key={s.id}
                  href={`/business/spend-get-desk?storeId=${encodeURIComponent(s.id)}`}
        ...
        <Card className="border-rose-200 bg-rose-50/50">
          <CardContent className="p-4 space-y-3">
            ...
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              {zh
                ? "补齐：长期代金 · 大奖倒计时 · 满赠。已有则刷新码。"
                : "Evergreen · grand countdown · spend & get. Refresh QR if exists."}
            </p>
            <Button
              type="button"
            ...
            {setupInfo && (
              <div className="grid grid-cols-2 gap-2 pt-1">
                <div className="rounded-xl bg-white border p-2 text-center">
                  ...
                  <img
                    src={`/api/campaign/qr?slug=${encodeURIComponent(setupInfo.slug)}&spendGet=1&from=table&size=200&format=png`}
                    alt="table"
                ...
                <div className="rounded-xl bg-white border p-2 text-center">
                  ...
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={`/api/campaign/qr?slug=${encodeURIComponent(setupInfo.slug)}&spendGet=1&from=counter&size=200&format=png`}
                    alt="counter"
                    className="w-full aspect-square object-contain"
                  />
                </div>
