...
export function SpendGetDeskClient({
  storeId,
  storeName,
  initialCampaignId,
  canComp,
  ...
  const load = useCallback(async () => {
    const res = await fetch("/api/business/spend-get/issue");
    if (!res.ok) return;
  ...
  async function onReceiptPhoto(file: File | null | undefined) {
    ...
    try {
      ...
      const res = await fetch("/api/business/spend-get/receipt-ocr", {
        method: "POST",
        body: form,
      });
      const j = await res.json();
        ...
        {campaigns.length === 0 ? (
          <Card className="border-amber-200 bg-amber-50">
            <CardContent className="p-4 text-sm text-amber-900">
              {zh
                ? "暂无满赠活动。请企业主到「活动券」开启默认活动。"
                : "No spend & get campaign. Owner should enable defaults from Offers."}
              {canComp && (
                <Link
