...
export default async function CouponsPage() {
  ...
  return (
    <div className="pb-4">
      <div className="px-4 py-3 border-b border-border flex items-center justify-between"><h1 className="text-lg font-semibold">{t("business.coupons.title", lang)}</h1><Link href="/business/coupons/new" className="px-3 py-1.5 bg-primary text-primary-foreground text-xs rounded-full">{t("business.coupons.create", lang)}</Link></div>
      <div className="px-4 mt-3 space-y-2">
        ...
        {coupons.length === 0 && <div className="text-center py-12 text-muted-foreground"><p className="text-3xl mb-2">🎫</p><p className="text-sm">{t("business.coupons.noCoupons", lang)}</p></div>}
      </div>
    </div>
  );
}
