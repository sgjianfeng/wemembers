    ...
    <div className="pb-4">
      <div className="bg-white border-b border-slate-100 px-4 py-3 sticky top-0 z-20">
        <div className="flex items-center justify-between">
          <div><p className="text-sm font-semibold text-slate-900">{user.businessName || "我的店铺"}</p><p className="text-xs text-slate-400">{new Date().toLocaleDateString("zh-CN", { weekday: "long", month: "long", day: "numeric" })}</p></div>
      ...
      <div className="px-4 mt-4">
        <div className="grid grid-cols-2 gap-3">
          {[{ icon: "👤", label: "会员数", value: memberCount.toString() },{ icon: "🎫", label: "进行中券", value: couponCount.toString() },{ icon: "✅", label: "今日领取", value: claimsToday.toString() },{ icon: "💎", label: "今日核销", value: redemptionsToday.toString() }].map(k => (
        ...
        <h3 className="text-sm font-semibold text-slate-900 mt-5 mb-2">⚡ 快捷操作</h3>
        <div className="grid grid-cols-2 gap-2">
          {[{ icon: "🎫", label: "发券", desc: "创建代金券", href: "/business/coupons/new" },{ icon: "👤", label: "会员", desc: "管理会员", href: "/business/members" },{ icon: "📷", label: "核销", desc: "扫码验证", href: "/business/scan" },{ icon: "🪙", label: "充值", desc: "购买Token", href: "/business/tokens" }].map(a => (
        ...
        <h3 className="text-sm font-semibold text-slate-900 mt-5 mb-2">📋 最近动态</h3>
        {recentActivity.length > 0 ? recentActivity.map((log) => (
          <div key={log.id} className="flex items-center justify-between px-3 py-2 bg-slate-50 rounded-lg text-sm mb-1"><span className="text-slate-600">✅ 核销 · 节省 ¥{log.amountSaved}</span><span className="text-xs text-slate-400">{timeAgo(log.redeemedAt)}</span></div>
        )) : <div className="text-center py-8 text-slate-400"><p className="text-3xl mb-2">📭</p><p className="text-sm">暂无动态</p></div>}
      </div>
    </div>
  );
}
