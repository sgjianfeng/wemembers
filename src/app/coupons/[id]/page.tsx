...
export default async function CouponDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!coupon || coupon.status !== "published") {
    return <div className="min-h-screen flex items-center justify-center"><div className="text-center text-muted-foreground"><p className="text-4xl mb-2">🎫</p><p className="text-sm">{t("coupon.detail.notFound", lang)}</p></div></div>;
  }
  ...
  return (
    <div className="pb-4 min-h-screen">
      ...
      <div className="bg-gradient-to-b from-[#1A6EFF] to-white px-4 pt-8 pb-6">
        <Card className="max-w-sm mx-auto overflow-hidden border-0 shadow-sm">
          ...
          <CardContent className="p-4 space-y-2">
            <div className="flex justify-between text-xs"><span className="text-muted-foreground">{t("coupon.detail.type", lang)}</span><span className="text-foreground">{typeLabel}</span></div>
            <div className="flex justify-between text-xs"><span className="text-muted-foreground">{t("coupon.detail.points", lang)}</span><span className="text-[#FF6B35] font-bold">{coupon.pointsRequired}⭐</span></div>
            <div className="flex justify-between text-xs"><span className="text-muted-foreground">{t("coupon.detail.minSpend", lang)}</span><span className="text-foreground">{coupon.minSpendCents > 0 ? `S$${(coupon.minSpendCents / 100).toFixed(0)}` : t("coupon.detail.noMinSpend", lang)}</span></div>
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">{t("coupon.detail.validity", lang)}</span>
              <span className={`${daysLeft <= 3 ? "text-red-500" : "text-foreground"}`}>
            ...
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">{t("coupon.detail.remaining", lang)}</span>
              <span className="text-foreground">{coupon.remainingQuantity !== null ? t("coupon.detail.remainingCount", lang, { remaining: String(coupon.remainingQuantity), total: String(coupon.totalQuantity || 0) }) : t("coupon.detail.unlimited", lang)}</span>
            </div>
            <div className="flex justify-between text-xs"><span className="text-muted-foreground">{t("coupon.detail.perCustomer", lang)}</span><span className="text-foreground">{t("coupon.detail.perCustomerCount", lang, { count: coupon.perCustomerLimit })}</span></div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="px-4 mt-4">
        {isCustomer ? (
          <>
            ...
            {pointsGap > 0 && (
              <div className="mb-3 p-3 bg-amber-50 dark:bg-amber-950/35 rounded-xl border border-amber-100 dark:border-amber-800/50">
                ...
                <div className="flex gap-2 mt-2">
                  <a href="/home" className="text-[11px] text-amber-600 dark:text-amber-400 underline">
                    {lang === "zh" ? "去签到 →" : "Check in →"}
                  </a>
                  <a href={`/shop/${coupon.business?.businessSlug || coupon.businessId}`} className="text-[11px] text-amber-600 dark:text-amber-400 underline">
                    {lang === "zh" ? "去消费 →" : "Shop →"}
                  </a>
                </div>
              </div>
          ...
          <div className="text-center p-4 bg-muted/50 rounded-xl">
            <p className="text-sm text-muted-foreground">{t("coupon.detail.loginPrompt", lang)}</p>
          </div>
        )}
      ...
      <div className="px-4 mt-5">
        <details className="bg-card rounded-xl border border-border overflow-hidden" open>
          <summary className="px-4 py-3 text-sm font-semibold text-foreground cursor-pointer hover:bg-muted/50">
            ...
            <div className="px-4 pb-3 space-y-2">
              {stores.map((store) => (
                <a key={store.id} href={`/store/${store.slug}`} className="flex items-center gap-3 py-2 border-b border-slate-50 last:border-0">
                  <span className="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-950/35 flex items-center justify-center text-sm shrink-0">🏢</span>
                  <div className="min-w-0">
                    <p className="text-xs font-medium text-foreground">{store.name}</p>
                    {store.address && <p className="text-[10px] text-muted-foreground truncate">{store.address}</p>}
                  </div>
      ...
      <div className="px-4 mt-3 mb-8">
        <button
          ...
          className="w-full py-3 bg-muted/50 rounded-xl text-xs text-muted-foreground hover:bg-muted transition-colors"
        >
