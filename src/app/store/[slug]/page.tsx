...
export default async function StorePage({
  ...
  const store = await prisma.store.findUnique({
    ...
    include: {
      business: {
        select: { id: true, businessName: true, businessCategory: true },
      },
    },
  });
      ...
      <div className="px-4 -mt-4 pb-8">
              ...
              {coupons.map((c) => {
                ...
                return (
                  <Link key={c.id} href={`/coupons/${c.id}`}>
                    <Card
                      className={`hover:border-[#1A6EFF]/30 border-l-4 border-l-[#FF6B35] ${
                        soldOut ? "opacity-50" : ""
                      }`}
                    >
        ...
        {!isLoggedIn && coupons.length > 0 && (
          <div className="mx-3 mt-4 p-4 bg-[#1A6EFF]/5 rounded-xl text-center">
            ...
            <Link
              href={`/auth/login?redirect=/store/${store.slug}`}
