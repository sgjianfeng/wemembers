...
describe("issueSpendGetGrant", () => {
    ...
    await expect(
      prisma.$transaction((tx) =>
        issueSpendGetGrant(tx, {
          ...
          channel: "receipt",
          receiptAmountCents: SPEND_GET_MIN_SPEND_CENTS - 1,
        })
      )
  ...
  it("redeem path issues coupon only (no gift draw voucher)", async () => {
    ...
    await prisma.campaign.update({
      where: { id: campaignId },
      data: { rulesSnapshot: buildSpendGetRulesSnapshot({ enabled: true }) },
    });

    const only = await prisma.$transaction((tx) =>
      issueSpendGetGiftOnly(tx, {
        campaignId,
        businessId,
        storeId,
        customerId: cust.id,
